...
async function createGist(token: string, content: string): Promise<void> {
  ...
  return await axios.post(
    ...
    {
      description: 'Auto Synced Mihomo Purity Runtime Config',
      public: false,
      files: { 'mihomo-purity.yaml': { content } }
    },
    {
      headers: {
        Accept: 'application/vnd.github+json',
...
export async function getGistUrl(): Promise<string> {
  ...
  const gist = gists.find((gist) => gist.description === 'Auto Synced Mihomo Purity Runtime Config')
  if (gist) {
    ...
    const gist = gists.find(
      (gist) => gist.description === 'Auto Synced Mihomo Purity Runtime Config'
    )
